...
/*
 * <p>A basic abstract class to facilitate implementing block cipher modes of operations.</p>
 *
...
public abstract class BaseMode implements IMode {
   ...
   private boolean testSymmetry(int ks, int bs) {
      try {
         ...
         map.put(CIPHER_BLOCK_SIZE, bs);
         map.put(STATE, ENCRYPTION);
         map.put(IV, iv);
         map.put(MODE_BLOCK_SIZE, bs);
