...
/*
 * <p>A basic abstract class to facilitate implementing symmetric key block ciphers.</p>
 *
...
public abstract class BaseCipher implements IBlockCipher, IBlockCipherSpi {
   ...
   public String name() {
      ...
      if (currentKey == null) {
         sb.append(8*defaultBlockSize);
      } else {
         sb.append(8*currentBlockSize);
      }
      return sb.toString();
   }
      ...
      synchronized(lock) {
         ...
         if (bs == null) { // no block size was specified.
            ...
            for (it = blockSizes(); it.hasNext(); ) {
               ok = (currentBlockSize == (Integer) it.next());
               if (ok) {
                  break;
               }
            }
   ...
   public boolean selfTest() {
      ...
      for (Iterator kit = keySizes(); kit.hasNext(); ) {
         ...
         for (bit = blockSizes(); bit.hasNext(); ) {
            if (!testSymmetry(ks, (Integer) bit.next())) {
               return false;
            }
         }
      }
